...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		//xzy 1.将resource包装成带编码格式的EnCodeResource
		//xzy 2.重载调用loadBeanDefinitions
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		//xzy 拿到当前线程已经加载过的所有encodedResource
		Set<EncodedResource> currentResources = this.resourcesCurrentlyBeingLoaded.get();

		//xzy 将当前encodedResource加入到ThreadLocal-set中，加入失败说明已经加载过了，不能重复加载，抛出异常
		...
		try (InputStream inputStream = encodedResource.getResource().getInputStream()) {
			//因为接下来要使用sax解析器解析xml，所以需要将输入流包装成inputSource, inputSource 是sax中表示资源的对象
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			//将resource转换成有层次结构的document对象
			Document doc = doLoadDocument(inputSource, resource);
			//将document 解析成bd并注册到bf中，返回新注册到bf中中bd数量
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		/**
		 * 1. EntityResolver？解析一个XML，SAX首先读取XML文档上的申明，根据声明去寻找响应的DTD/XSD定义，
		 * 		以便对文档进行校验。默认的寻找规则通过下载xml上的声明，再进行校验，并且下载的过程漫长且不可控，
		 * 		下载失败还会抛错。EntityResolver 是项目本身提供的一个如何寻找声明的方法，避免了从网络下载。
		 * 2. getValidationModeForResource 获取验证模式
		 */
	...
	protected int getValidationModeForResource(Resource resource) {
		...
		//条件成立：说明set设置过默认值，一般情况下不会这样做，都是使用默认值
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 创建一个 BeanDefinitionDocumentReader，每个document对象都会创建一个BeanDefinitionDocumentReader对象
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();

		//getRegistry() 返回程序创建的 BeanFactory 实例。countBefore：解析改doc之前，bf中已有的bd数量。
		int countBefore = getRegistry().getBeanDefinitionCount();
		//解析doc，并且注册到bf中。
		//XmlReaderContext：包含最主要的参数是 当前this-> XmlBeanDefinitionReader -> bf
